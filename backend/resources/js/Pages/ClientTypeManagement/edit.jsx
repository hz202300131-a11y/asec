...
const EditClientType = ({ clientType, setShowEditModal }) => {
  if (!clientType) {
    return null;
  }
  ...
  const { data, setData, put, errors, processing } = useForm({
    ...
    is_active: clientType?.is_active ?? true,
  });
  ...
  const handleSubmit = (e) => {
    ...
    put(route('client-type-management.update', clientType.id), {
      preserveScroll: true,
      preserveState: true,
      only: ['clientTypes'],
      onSuccess: () => {
  ...
  return (
    <Dialog open onOpenChange={setShowEditModal}>
      <DialogContent className="w-[95vw] max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-zinc-800">Edit Client Type</DialogTitle>
          <DialogDescription className="text-zinc-600">
            Update the details for this client type below.
          </DialogDescription>
        </DialogHeader>
